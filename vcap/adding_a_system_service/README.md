...
![service_provisioning.png](images/service_provisioning.png)
...
5. Download and extract the implementation of the [echo service provisioner](support/echo_sp.zip) to the services host. Then move the implementation files and config files to the appropriate locations:
...
services`. This will output something like this:
   ...
   ![echo_service.png](images/echo_service.png)
...
2. Push a test application and bind the provisioned echo service.
   ...
   Download the test application's [.war file](support/testapp.war), or compile from the [source code](support/testapp_src.zip). Place it in an empty folder and deploy with `vmc push`:
...
really is a listening service. Without doing so the application will return an error when attemting to access the echo service. So let's start the service: download the [echo service jar](support/echo_service.jar) to the `host` listed in `echo_node.yml`, or compile it from the [source code](support/echo_service_src.zip). Then execute the following:
