...
1. You need to run the BOSH inception from Ubuntu

1. Install some core packages on Ubuntu that the BOSH deployer depends on.

		sudo apt-get -y install libsqlite3-dev genisoimage

1. Ruby 1.9.2 or later. It is recommend that you install into an empty gemset (or similar).
...
The files in the `deployments` directory need to be saved if you later want to be able to update your micro BOSH instance. They are all text files, so you can commit them to a git repository to make sure they are safe in case your inception VM goes away.
...
**Note that this manifest has passwords that are publicly known, but they can and should be over ridden during the deployment**
...
Before you can deploy, you need to upload the stemcell you built to your AWS inception VM:
